...
const ChatScreen = ({ user, navigation, route }) => {
  ...
  const getMessageActions = async () => {
    try {
      const result = await pubnub.getMessageActions({
        channel: route.params.channnelName,
      });
      result.data.map(async (value) => {
        if (value.uuid !== user.phone) {
          try {
            const result = await pubnub.removeMessageAction({
              channel: route.params.channnelName,
              messageTimetoken: value.messageTimetoken,
              actionTimetoken: value.actionTimetoken,
            });
            useDispatch(setUnreadMessage([]));
          } catch (status) {
            console.log(status);
          }
        }
  ...
  const handleSubmit = () => {
    ...
    if (input.length > 0) {
      ...
      PushNotification(
        userData.devise_token,
        `${route.params.from?.name} sent you a message`,
        "New Message"
      );
      // addMessageAction();
    }
  ...
  const addMessageAction = async (envelope) => {
