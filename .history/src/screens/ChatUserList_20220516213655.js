...
const ChatUserList = ({ user, navigation, route, unreadMessage }) => {
        ...
        if (res.status) {
          ...
          res.data.map(async (value) => {
            try {
              ...
              useDispatch(setUnreadMessage(blankArray));
            } catch (status) {
              console.log(status);
            }
          });
  ...
  const unreadCount = (channel) => {
    console.log(unreadMessaged, channel);
    var isUnread = unreadMessaged.some(
